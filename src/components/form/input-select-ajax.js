...
export default class FormInputSelectAjax extends Component {
  ...
  formatState(type, state) {
    ...
    var $state = $(
      `<span class="${type === "selectmultiple" ? "multiple" : ""}">${
        state.text
      }</span>`,
    )
    return $state
          ...
          config.ajax = {
            ...
            data: (params) => {
              ...
              if (this.props.filter) {
                ...
                filters = [
                  [this.props.column, "like", params.term],
                  ["AND"],
                  JSON.parse(filter),
                ]
              }
    ...
    return isFilter ? (
      <div className="col-xs-12 col-sm-12 col-md-6 col-lg-4">
        <div className="col-xs-4">
          <label className="text-label-filter font-weight-bold">
            {this.props.label}
          </label>
