...
const EmployeeForm = (props) => {
  ...
  const onSubmit = async (step, values, setSubmitting) => {
    let formId = props.match.params.id
    const Data = {
...
const TabNav = ({moveStep}) => {
  return (
    <Nav variant="pills" className="flex-column nav-side">
      <Nav.Item>
        <Nav.Link eventKey="general-information" onClick={() => moveStep(0)}>
          <div>
      ...
      <Nav.Item>
        <Nav.Link eventKey="emergency-contacts" onClick={() => moveStep(1)}>
          <div>
      ...
      <Nav.Item>
        <Nav.Link eventKey="employment" onClick={() => moveStep(2)}>
          <div>
...
const StepContent = ({children, eventKey}) => children

const TabContents = ({children, initialValues, onSubmit, formId}) => {
  const history = useHistory()
  const isView = useQuery().get("action") === "view"
  ...
  const handleSubmit = async (values, bag) => {
    ...
    // }
    if (isLastStep) {
      return onSubmit(values, bag.setSubmitting);
    } else {
      bag.setTouched({});
      step.props.onSubmit()
      next(values);
