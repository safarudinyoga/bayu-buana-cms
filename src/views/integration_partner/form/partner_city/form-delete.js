...
function DeleteForm(props) {
  ...
  useEffect(async () => {
    ...
    if(formId) {
      try {
        let {data} = await API.get(`/master/integration-partners/${props.match.params.id}/cities` + "/" + formId)
  ...
  useEffect(() => {
	...
	const onSubmit = async (values, a) => {
		try {
      let res = await API.delete(`/master/integration-partners/${props.match.params.id}/cities/` + id)
      dispatch(setModalDelete({show: false, id: null, disabled_form: false}))
      dispatch(
				setAlert({
				  message: ` Record 'Partner City Name: ${formValues.city_name} was successfully deleted.`,
				}),
      )
		} catch(e) {
			dispatch(
    ...
    <>
      ...
      <img src={errorIcon} className="mr-2" alt="new" />
        ...
        Are you sure you want to delete 'Partner City Name: {formValues.city_name}'?
        </span>
      </div>
