...
function OTP(props) {
  ...
  return (
    <>
      <p className={`mid-title p-0 ${corporateLogin ? "mb-3" : "mb-1"}`}>
      ...
      >
        {({ dirty, handleSubmit, isSubmitting }) => (
          <Form onSubmit={handleSubmit}>
            <FormValidate
              label={corporateLogin ? "Enter OTP Code from Email" : "OTP Code"}
              name="code"
              type={passType}
              maxLength={256}
              placeholder={corporateLogin ? "" : "Enter your OTP code"}
              endIcon={
                corporateLogin
                  ? false
                  : () => (
