...
const SecuritySettings = (props) => {
  ...
  useEffect(async () => {
    try {
      let res = await api.get("/user/accounst")
  ...
  return (
    <Formik
      ...
      onSubmit={async (values, { setSubmitting, resetForm }) => {
        ...
        try {
          let res = await api.put("user/accounts", formatted)
          openSnackbar(`Password has been successfully changed.`)
          //   signout()
        } catch (e) {}
            ...
            <Card style={{ marginBottom: 0 }}>
              <Card.Body>
                ...
                <div style={{ padding: "0 15px 40px 0" }}>
                  <Button style={{ background: "#E84D0E", border: "#E84D0E" }}>
                  ...
                  <FormValidate
                    ...
                    endIcon={() => (
                      <i
                        onClick={() =>
                          setOldPassType(
                            oldPassType === "text" ? "password" : "text",
                          )
                        }
                        className={`fa ${
                          oldPassType === "password" ? "fa-eye-slash" : "fa-eye"
                        }`}
                      ></i>
                    )}
                  />
                  <FormValidate
                    ...
                    endIcon={() => (
                      <i
                        onClick={() =>
                          setNewPassType(
                            newPassType === "text" ? "password" : "text",
                          )
                        }
                        className={`fa ${
                          newPassType === "password" ? "fa-eye-slash" : "fa-eye"
                        }`}
                      ></i>
                    )}
                  />
                  <FormValidate
                    ...
                    endIcon={() => (
                      <i
                        onClick={() =>
                          setConfirmPassType(
                            confirmPassType === "text" ? "password" : "text",
                          )
                        }
                        className={`fa ${
